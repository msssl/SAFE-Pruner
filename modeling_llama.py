...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2) # [1, 32, 605, 128]
        key_states = key_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2) # [1, 32, 605, 128]
        value_states = value_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2) # [1, 32, 605, 128]
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _redundancy_minization(self, visual_feature_vectors, num_keep, cosine_matrix=None):
        ...
        if cosine_matrix is None:
            ...
            norm_matrix = visual_feature_vectors / norm.clamp(min=1e-8)
            cosine_similarity = torch.mm(norm_matrix, norm_matrix.t())
            cosine_matrix = 1.0 - cosine_similarity
        ...
        selected = torch.empty(num_keep, dtype=torch.long, device=device)

        topk_vals = torch.topk(cosine_matrix, 2, dim=0, largest=False).values
        scores = topk_vals[1]
        first_idx = torch.argmax(scores)
        selected[0] = first_idx
        min_dists = torch.min(min_dists, cosine_matrix[first_idx])

        for i in range(1, num_keep):
            next_idx = torch.argmax(min_dists)
            selected[i] = next_idx
            min_dists = torch.min(min_dists, cosine_matrix[next_idx])


        return selected
    
    def forward_safepruner(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        ...
        fastv_config["fastv_k"] = int(os.environ.get("FASTV_K", 3))
        fastv_config["fastv_r"] = float(os.environ.get("FASTV_R", 0.75))
        ...
        causal_mask = self._update_causal_mask(attention_mask, inputs_embeds, cache_position, past_seen_tokens, False)
        # embed positions
        hidden_states = inputs_embeds
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if use_cache:
                if hidden_states.shape[1] != 1:
                    ...
                    if layer_idx == 1:
                        # compute pruned tokens, generate fastv sign (skip if keyframe)
                        last_layer_attention = layer_outputs[1] #[1,32,605,605]
                        ...
                        ### avoid uniform distribution of attention(layer 0)
                        attn_slice = "-1"
                        ...
                        last_layer_attention_avg_last_tok_image = last_layer_attention_avg_last_tok[FASTV_image_token_start_index:FASTV_image_token_start_index+FASTV_image_token_length] # [512]
                        if layers_attention_forecast is not None:
                            lambda_ = float(os.getenv("LAMBDA", 0.5))
                            forecast_attention = layers_attention_forecast[0]

                            num_primary_image_tokens = forecast_attention.shape[-1]  // 2
                            
                            ### cosine similarity
                            import torch.nn.functional as F
                            cosine_sim = F.cosine_similarity(
                                forecast_attention.unsqueeze(0), 
                                last_layer_attention_avg_last_tok_image.unsqueeze(0), 
                                dim=1
                            ).item()
                            
                            keyframe_similarity_threshold = float(os.getenv("KEYFRAME_SIMILARITY_THRESHOLD", 0.95))
                            is_keyframe = cosine_sim < keyframe_similarity_threshold
                    ...
                    if layer_idx<FASTV_k:
                        ...
                        if is_keyframe:
                            ...
                            avg_attention = layers_attention_forecast.mean(dim=0)
                            
                            
                            
                            last_layer_attention_avg_last_tok_image = last_layer_attention_avg_last_tok_image + lambda_ * avg_attention
                        ...
                        # Keyframe: no pruning, use original causal mask
                        pruned_attention_mask = causal_mask
                else:
                    pruned_attention_mask = causal_mask
            ...
            if hidden_states.shape[1] !=1:     
                ...
                m = self.layers[layer_idx].mlp.up_proj.out_features         # intermediate size of the FFN
                FLOPs_current = 4 * n * (d**2) + 2 *(n**2) * d + 3*n*d*m 
                self.all_FLOPs += FLOPs_current  # n 596 d 4096 m 11008
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        cache_position: Optional[torch.LongTensor] = None,
        is_keyframe: Optional[bool] = False,
        avg_attention: Optional[torch.Tensor] = None,
    ...
    def forward_safepruner(
        ...
        # decoder outputs consists of (dec_features, layer_state, dec_hidden, dec_attn)
        outputs = self.model.forward_safepruner(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
