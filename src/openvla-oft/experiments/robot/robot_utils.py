...
def get_action_safepruner(
    ...
    with torch.no_grad():
        if cfg.model_family == "openvla":
            action, last_caches, result_image, metrics, keyframe_cache = get_vla_action_safepruner(
                cfg=cfg,
                vla=model,
                processor=processor,
                obs=obs,
    ...
    return action, last_caches, result_image, metrics, keyframe_cache
