...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    if getattr(cfg, "use_fp8", False):
        from experiments.robot.fp8_utils import apply_fp8

        vla = apply_fp8(
            vla,
            fp8_dtype=getattr(cfg, "fp8_dtype", "e4m3fn"),
            quantize_activations=getattr(cfg, "fp8_quantize_activations", True),
            device=DEVICE,
            module_label="VLA model",
        )
...
def get_vla_action(
    ...
    if action_head is None:
        ...
        if os.getenv("visualize_attention") == "True":
            action, _, last_caches, attentions = vla.predict_action(
                **inputs,
            ...
            vis_layers_str = os.getenv("VIS_LAYERS", "2")

            if "-" in vis_layers_str:
                start, end = map(int, vis_layers_str.split("-"))
                vis_layers = list(range(start, end + 1))
            elif "," in vis_layers_str:
                vis_layers = [int(x.strip()) for x in vis_layers_str.split(",")]
            else:
                vis_layers = [int(vis_layers_str)]

            multi_layer_attention_images = {}  # {layer_idx: attention_tensor}
            
            for layer_idx in vis_layers:
                if layer_idx >= len(attentions):
                    print(f"Warning: Layer {layer_idx} out of range (max: {len(attentions)-1})")
                    continue
                    
                last_layer_attention = attentions[layer_idx]  # [1,32,605,605]
                last_layer_attention_avg = torch.mean(last_layer_attention, dim=1)[0]  # [605,605]

                attn_slice = os.getenv("ATTN_SLICE", "-1")
                if ":" in attn_slice:
                    start, end = map(int, attn_slice.split(":"))
                    if end == 0:
                        last_layer_attention_avg_last_tok = last_layer_attention_avg[start:].mean(dim=0)
                    else:
                        last_layer_attention_avg_last_tok = last_layer_attention_avg[start:end].mean(dim=0)
                else:
                    idx = int(attn_slice)
                    last_layer_attention_avg_last_tok = last_layer_attention_avg[idx]
                ...
                multi_layer_attention_images[layer_idx] = last_layer_attention_avg_last_tok_image

            from .vla_cache_utils import draw_patches_overlay_attention, draw_patches_overlay_attention_every_image_3tier,draw_patches_overlay_attention_every_image_3tier_tier0_only
            result_images_dict = {}  # {layer_idx: [vis_primary, vis_wrist]}
            
            for layer_idx, attention_tensor in multi_layer_attention_images.items():
                # vis_primary, vis_wrist = draw_patches_overlay_attention(
                #     result_image[0], result_image[1], attention_tensor
                # )
                # vis_primary, vis_wrist = draw_patches_overlay_attention_every_image_3tier(
                #     result_image[0], result_image[1], attention_tensor
                # )
                vis_primary, vis_wrist = draw_patches_overlay_attention_every_image_3tier_tier0_only(
                    result_image[0], result_image[1], attention_tensor
                )
                result_images_dict[layer_idx] = [vis_primary, vis_wrist]
            
            if len(result_images_dict) == 1:
                result_image = list(result_images_dict.values())[0]
            else:
                result_image = result_images_dict
        else:
    ...
    action_list = [action[i] for i in range(min(len(action), cfg.num_open_loop_steps))] #len(action_list)=8 action_list[0] array(7,)
    

    if isinstance(result_image, dict):
        result_image = {
            layer_idx: [np.array(img) for img in images]
            for layer_idx, images in result_image.items()
        }
    else:
...
def get_vla_action_safepruner(
...
) -> Tuple[List[np.ndarray], Any, Any, Dict[str, Any], Optional[Dict[str, Any]]]:
    """
    Generate action predictions with keyframe mechanism for adaptive pruning.
    ...
    Args:
        ...
        noisy_action_projector: Optional noisy action projector for diffusion
        use_film: Whether to use FiLM
        last_caches: Previous KV caches
        keyframe_cache: Cache storing keyframe information
        
    Returns:
        Tuple of (actions, caches, result_images, metrics, keyframe_cache)
    """

    # Initialize keyframe cache if needed
    ...
    if all_images:
        ...
        primary_pixel_values = inputs["pixel_values"]
        all_wrist_pixel_values = [wrist_inputs["pixel_values"] for wrist_inputs in all_wrist_inputs]
        inputs["pixel_values"] = torch.cat([primary_pixel_values] + all_wrist_pixel_values, dim=1) ### [1,12,244,244]
    ...
    # Generate action with keyframe flag
    if action_head is None:
        action, _ = vla.predict_action(**inputs, unnorm_key=cfg.unnorm_key, do_sample=False)
    else:
        action, _, last_caches, attentions, is_keyframe = vla.predict_action_safepruner(
            **inputs,
            unnorm_key=cfg.unnorm_key,
            do_sample=False,
            proprio=proprio,
    ...
    if is_keyframe:
        ...
        s,e,idx = None, None, None
        if ":" in attn_slice:
            s, e = map(int, attn_slice.split(":"))
        else:
            idx = int(attn_slice)
        

        attention_probs = []
        layers = [int(x) for x in os.getenv("AVG_LAYERS", "11,21").replace("-", ",").split(",")]
        for l in layers:
            attn = attentions[l] # attn.shape [1, 32, 596, 596]
            if l == 0:
                attn_avg = attn.mean(dim=1)[0][-1][1:513] ### avoid uniform distribution of attention(layer 0)
            else:
                if idx is not None:
                    attn_avg = attn.mean(dim=1)[0][idx][1:513]  # image token [512]
                else:
                    attn_avg = attn.mean(dim=1)[0][s:e].mean(dim=0)[1:513]
            attention_probs.append(attn_avg)
        attention_probs = torch.stack(attention_probs)
        
        keyframe_cache["visual_patches"] = all_images.copy()
        keyframe_cache["attention_probs"] = attention_probs 
    
    
    return action_list, last_caches, result_image, metrics, keyframe_cache
